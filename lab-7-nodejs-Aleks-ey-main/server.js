...
app.get('/team_stats', function(req, res) {
	var gamesQ = 'select * from football_games;'; // Investigate why the parameter is named "color_selection"
	var WQ = 'select count(*) from football_games where home_score > visitor_score;';// Write a SQL query to retrieve the colors from the database
	var LQ = 'select count(*) from football_games where home_score < visitor_score;';// Write a SQL query to retrieve the color message for the selected color
	db.task('get-everything', task => {
        return task.batch([
            task.any(gamesQ),
            task.any(WQ),
    ...
    .then(data => {
    	res.render('pages/team_stats',{
				my_title: "Team Stats",
				allGames: data[0],// Return the color options
				W: data[1],// Return the color choice
				L: data[2]// Return the color message
			})
    })
    .catch(err => {
            console.log('error', err);
